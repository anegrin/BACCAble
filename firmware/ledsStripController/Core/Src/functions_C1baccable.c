...
#include "dashboard.h"
	...
	void sendDashboardPageToSlaveBaccable(float param){
		...
		switch(uds_params_array[function_is_diesel_enabled][dashboardPageIndex].reqId){
			...
			case 0x19:
				...
				break;
			default:
				//start: temporary code to show new dashboard proto usage
				if (dashboardPageIndex == 0) {
					encodeToDashboardMessage(uartTxMsg, UART_BUFFER_SIZE, 1, HP_ITEM_ID, param, 0.0);
					break;
				}
				if (dashboardPageIndex == 1) {
					encodeToDashboardMessage(uartTxMsg, UART_BUFFER_SIZE, 1, TORQUE_ITEM_ID, param, 0.0);
					break;
				}
				//end: temporary code to show new dashboard proto usage
		...
		if (dashboardPageIndex > 1) {//temporary check to show new dashboard proto usage, will be removed
	...
	void clearDashboardBaccableMenu(){
		encodeToDashboardMessage(uartTxMsg, UART_BUFFER_SIZE, 1, CLEANUP_ITEM_ID, 0.0, 0.0);
		addToUARTSendQueue(uartTxMsg, UART_BUFFER_SIZE);
